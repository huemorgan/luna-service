...
import {
  API, BLOCK_MESSAGES, credits, fmtDate, fmtDateTime, getJson, usd, apiError,
} from './api';
import type {
...
const GRANT_CATEGORY_LABELS: Record<string, string> = {
  paid: 'Paid', bonus: 'Bonus', gift: 'Gift', free: 'Free', topup: 'Top-up',
};
...
export default function BillingPage() {
  ...
  const monthlyPlans = products?.products.filter(p => p.kind === 'subscription' && p.interval === 'month') ?? [];
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--ink)' }}>
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        ...
        <h2 className="text-2xl font-bold mb-6" style={{ color: 'var(--text)' }}>Credits &amp; usage</h2>
        ...
        {summary && (
          <>
            {/* Block banners derived from the account state. */}
            ...
            {grants && grants.length > 0 && (
              <section className="rounded-2xl p-5 border mb-8" style={card}>
                ...
                <p className="text-xs mt-3" style={{ color: 'var(--text-dim)' }}>
                  Credits burn cheapest-first: free → gift → bonus → top-up → paid.
                </p>
              </section>
            )}
            ...
            {products && (
              <section className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {monthlyPlans.map(p => (
                    <div key={p.key} className="rounded-2xl p-5 border" style={card}>
                      ...
                      <div className="text-sm" style={{ color: 'var(--text)' }}>
                        {p.paid_credits.toLocaleString()} credits monthly
                        {p.bonus_credits > 0 && <> + {p.bonus_credits.toLocaleString()} bonus</>}
                      </div>
                      <button disabled
                        className="mt-4 w-full px-4 py-2 rounded-xl text-sm font-semibold cursor-not-allowed opacity-50"
                        style={{ background: 'var(--moon)', color: 'var(--ink)' }}
                        title="Online payments are rolling out">
                        Coming soon
                      </button>
                    </div>
                  ))}
